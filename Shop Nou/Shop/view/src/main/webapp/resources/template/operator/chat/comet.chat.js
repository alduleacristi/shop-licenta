...
(function($) {
	$.cometChat = {
		...
		memberListContainerID : undefined, // 'id' of a 'div' or 'span' tag
											// which keeps list of online users.

		/**
		 * This method can be invoked to disconnect from the chat server. When
		 * user logging off or user close the browser window, user should be
		 * disconnected from cometd server.
		 */
		leave : function() {
		...
		/**
		 * Handshake with the server. When user logging into your system, you
		 * can call this method to connect that user to cometd server. With that
		 * user will subscribe with tow channel. '/chat/demo' and
		 * '/members/demo' and start to listen to those channels.
		 */
		...
		/**
		 * Updates the members list. This function is called when a message
		 * arrives on channel /chat/members
		 */
		members : function(message) {
			...
			$
					.each(
							message.data,
							function() {
								if ($.cometChat.loginUserName == this) { // login
																			// user
									sb[sb.length] = "<span style=\";color: #330066;\">"
											+ this + "</span><br>";
								} else { // peer users
									sb[sb.length] = "<span onclick=\"javascript:createWindow('"
											+ $.cometChat.loginUserName
											+ "', '"
											+ this
											+ "');\"  style=\"cursor: pointer;color: #CC0066;\">"
											+ this + "</span><br>";
								}
							});
		...
		/**
		 * This function will be invoked every time when '/chat/demo' channel
		 * receives a message.
		 */
		receive : function(message) {
			...
			if ($.cometChat.loginUserName == toUser) {
				...
				chatReceivingWindow.appendMessage(fromUser, text,
						$.cometChat.loginUserName);
			}
			...
			if ($.cometChat.loginUserName == fromUser) {
				...
				chatSendingWindow.appendMessage(fromUser, text,
						$.cometChat.loginUserName);
			}
		},
		...
		_connectionClosed : function() {
			/*
			 * $.cometChat.receive({ data: { user: 'system', chat: 'Connection
			 * to Server Closed' } });
			 */
		},
		...
		_subscribe : function() {
			$.cometChat._chatSubscription = $.cometd.subscribe('/chat/demo',
					$.cometChat.receive); // channel handling chat messages
			$.cometChat._membersSubscription = $.cometd.subscribe(
					'/members/demo', $.cometChat.members); // channel handling
															// members.
		},
